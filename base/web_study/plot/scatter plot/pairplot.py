...
sns.pairplot(data = df_train, vars = ['hour', 'attendance', 'weight', 'iq', 'library'])
plt.show()

sns.pairplot(data=df_train[['hour', 'attendance', 'weight', 'iq', 'library']])
plt.show()

sns.pairplot(data=df_train, hue='pass', diag_kind='hist')
plt.show()

df_temp = df_train.copy()
df_temp['pass'] = df_temp['pass'].replace([0,1], ['fail','pass'])
sns.pairplot(data=df_temp, hue='pass', diag_kind='hist')
plt.show()